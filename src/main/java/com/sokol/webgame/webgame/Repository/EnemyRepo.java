...
public interface EnemyRepo extends JpaRepository<Enemy, Long> {
    @Query(value = "SELECT * FROM enemy WHERE type = 'Enemy' ORDER BY RANDOM() LIMIT 1", nativeQuery = true)
    Enemy findRandomEnemy();

    @Query(value = "SELECT * FROM enemy WHERE type = 'Boss' ORDER BY RANDOM() LIMIT 1", nativeQuery = true)
